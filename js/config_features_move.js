...
$(window).load(function () {
    ...
    function showFeaturedBox () {
         if ($(window).width() < 1000) {
            ...
            if ($(this).hasClass("feature_left_box")){

                $(this).siblings().animate({height: 0, padding: 0}, 800).addClass('transformXNeg1000');
                $(".feature_right_box").parent(".features-section-row").animate({width: 0}, 0);
                $(this).parent(".features-section-row").animate({width: '58.3333%'}, 0).addClass("col-sm-8").removeClass("col-sm-4 col-md-4");
                $(this).find("p").animate({fontSize: '28px', lineHeight: '45px'}, 0);
            } else if ($(this).hasClass("feature_right_box")) {
                var that = this;
                $(this).siblings().animate({height: 0, padding: 0}, 800);
                $(this).siblings().addClass('transformX1000');
                $(".feature_left_box").parent(".features-section-row").animate({width: 0}, 0);
                $(this).parent(".features-section-row").animate({width: '58.3333%'}, 0).addClass("col-sm-8").removeClass("col-sm-4 col-md-4");
                $(this).find("p").animate({fontSize: '28px', lineHeight: '45px'}, 0);
            } 
        }
            
        $(this).one('click', hideFeaturedBox);
    }
    function hideFeaturedBox () {
        if ($(window).width() < 1000) {
            return;
        } else {
            ...
            if ($(this).hasClass("feature_left_box")){
                var that = this;
                $(this).siblings().animate({padding: '10px', height: '100%'}, 100).removeClass('transformXNeg1000').css({height: 'auto'});
                $(".feature_right_box").parent(".features-section-row").animate({width: '33.3333%', height: '100%'}, 0);
                $(this).parent(".features-section-row").animate({width: '33.3333%'}, 0, function () {$(that).parent(".features-section-row").addClass('col-sm-12 col-md-4').removeClass('col-sm-8').attr({style: ''});});
                $(this).find('p').animate({fontSize: '14px', lineHeight: '26px'}, 0);
                
            } else if ($(this).hasClass("feature_right_box")) {
                var that = this;

                $(this).siblings().stop().animate({padding: '10px', height: '100%'}, 100).removeClass('transformX1000').css({height: 'auto'});
                $(".feature_left_box").parent(".features-section-row").css({height: 'auto'}).removeClass('transformXNeg1000');
                $(".feature_left_box").parent(".features-section-row").addClass('col-sm-12 col-md-3').animate({width: '33.3333%'}, 0);
                $(this).parent(".features-section-row").animate({width: '33.3333%'}, 0, function () {$(that).parent(".features-section-row").addClass('col-sm-12 col-md-4').removeClass('col-sm-8').attr({style: ''});});
                $(this).find('p').animate({fontSize: '14px', lineHeight: '26px'});
            }
        }     

        $(this).one('click', showFeaturedBox);
    }
    if ($(window).width() > 1000) {
        $('.features-section-box-jquery').one('click',showFeaturedBox);
    }
    //if ($(window).width() < 980) {
      //  $(".features-section-row").animate({width: '100%'}, 140);
    //}
    /* *****************************************************************
